...
#define st506_rddt_wrap_target 0
#define st506_rddt_wrap 0
#define st506_rddt_pio_version 1

static const uint16_t st506_rddt_program_instructions[] = {
            //     .wrap_target
    0x6001, //  0: out    pins, 1
            //     .wrap
};
...
#if !PICO_NO_HARDWARE
static const struct pio_program st506_rddt_program = {
    .instructions = st506_rddt_program_instructions,
    .length = 1,
    .origin = -1,
    .pio_version = st506_rddt_pio_version,
#if PICO_PIO_VERSION > 0
    .used_gpio_ranges = 0x0
#endif
};

static inline pio_sm_config st506_rddt_program_get_default_config(uint offset) {
    pio_sm_config c = pio_get_default_sm_config();
    sm_config_set_wrap(&c, offset + st506_rddt_wrap_target, offset + st506_rddt_wrap);
...
static inline void st506_rddt_program_stop(PIO pio, uint sm) {
	pio_sm_set_enabled(pio, sm, false);
	if (st506_rddt_chan >= 0) {
		dma_channel_cleanup(st506_rddt_chan);
		dma_channel_unclaim(st506_rddt_chan);
		st506_rddt_chan = -1;
	}
}
