...
#define st506_wrgt_wrap_target 0
#define st506_wrgt_wrap 3
#define st506_wrgt_pio_version 1

static const uint16_t st506_wrgt_program_instructions[] = {
            //     .wrap_target
...
#if !PICO_NO_HARDWARE
static const struct pio_program st506_wrgt_program = {
    .instructions = st506_wrgt_program_instructions,
    .length = 4,
    .origin = -1,
    .pio_version = st506_wrgt_pio_version,
#if PICO_PIO_VERSION > 0
    .used_gpio_ranges = 0x0
#endif
};

static inline pio_sm_config st506_wrgt_program_get_default_config(uint offset) {
    pio_sm_config c = pio_get_default_sm_config();
    sm_config_set_wrap(&c, offset + st506_wrgt_wrap_target, offset + st506_wrgt_wrap);
...
static int st506_wrgt_ctrl_chan = -1;
static int st506_wrgt_work_chan = -1;
static int st506_wrgt_trig_chan = -1;
static uint st506_wrgt_wrdt_mask;
static uint st506_wrgt_trigger_ctrl_block[8][2];
static uint st506_wrgt_abort_ctrl_block[4][2];
...
static inline void st506_wrgt_program_start(
	...
	uint rddt_chan,
	uint wrdt_chan,
	const volatile void *wrdt_pio_rxf,
	volatile void *wrrmc_pio_txf,
	volatile void *wrrma_pio_txf
) {
	...
	// 2. Write Channel's reload transfer count -> WRRMC PIO TX FIFO
	st506_wrgt_abort_ctrl_block[1][0] = (uint)&dma_debug_hw->ch[wrdt_chan].dbg_tcr;
	st506_wrgt_abort_ctrl_block[1][1] = (uint)wrrmc_pio_txf;
	// 3. Write Channel's write address -> WRRMA PIO TX FIFO
	st506_wrgt_abort_ctrl_block[2][0] = (uint)&dma_hw->ch[wrdt_chan].write_addr;
	st506_wrgt_abort_ctrl_block[2][1] = (uint)wrrma_pio_txf;
	// 4. Finish by writing zero to the trigger register
	st506_wrgt_abort_ctrl_block[3][0] = 0;
	st506_wrgt_abort_ctrl_block[3][1] = 0;
	// Trigger Channel
...
static inline void st506_wrgt_program_stop(PIO pio, uint sm) {
	pio_sm_set_enabled(pio, sm, false);
	if (st506_wrgt_trig_chan >= 0) {
		dma_channel_cleanup(st506_wrgt_trig_chan);
		dma_channel_unclaim(st506_wrgt_trig_chan);
		st506_wrgt_trig_chan = -1;
	}
	if (st506_wrgt_work_chan >= 0) {
		dma_channel_cleanup(st506_wrgt_work_chan);
		dma_channel_unclaim(st506_wrgt_work_chan);
		st506_wrgt_work_chan = -1;
	}
	if (st506_wrgt_ctrl_chan >= 0) {
		dma_channel_cleanup(st506_wrgt_ctrl_chan);
		dma_channel_unclaim(st506_wrgt_ctrl_chan);
		st506_wrgt_ctrl_chan = -1;
	}
}
